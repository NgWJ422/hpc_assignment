...
const bool VERBOSE_OUTPUT = true;  // Set to true to enable detailed output
...
int main() {
    ...
    try {
        ...
        if (avg_hybrid < avg_gpu && avg_hybrid < avg_cpu)
            std::cout << "Hybrid is the fastest.\n";
        else if (avg_gpu < avg_cpu)
            std::cout << "GPU-only is the fastest.\n";
        else
            std::cout << "CPU-only is the fastest.\n";

        // === Export to CSV ===
        std::ofstream csv_file("../../../../plot_timer/timing_data3.csv");
		if (!csv_file.is_open()) {
			std::cerr << "Error opening file for writing: timing_data.csv\n";
			return 1;
		}
        csv_file << "Iteration,CPU,GPU,Hybrid\n";
        for (int i = 0; i < ITERATIONS; ++i) {
            csv_file << (i + 1) << "," << times_cpu[i]
                << "," << times_gpu[i]
                    << "," << times_hybrid[i] << "\n";
        }
        csv_file.close();
        std::cout << "\n[CSV Export] Timing data saved to 'timing_data3.csv'.\n";
